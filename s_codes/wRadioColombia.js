...
/* Config cookie domain to www.wradio.com.co */
...
s.linkInternalFilters = "javascript:,wradio.com.co"
s.linkLeaveQueryString = false
s.linkTrackVars = "None"
s.linkTrackEvents = "None"

/* Form Analysis Plugin */
s.formList = "dataform,loginForm"
...
if (arraySite[0] == "www") { s.siteID = "wradio.com.co"; }
else { s.siteID = site; }
s.defaultPage = "";
s.queryVarsList = "";
s.pathExcludeDelim = ";";
...
//if (typeof (detallePagina) == 'undefined') detallePagina = document.title;
...
if (typeof (userId) == 'undefined') userId = ''; reg = "no registrado"; if (userId != '') { reg = "registrado" }
...
function s_doPlugins(s) {
    ...
    if (!s.campaign) {
        ...
        if (s.getQueryParam('afl') != '') s.campaign = s.getQueryParam('afl'); // afiliación
    ...
    //Internal Campaigns Trackings
    if (s.getQueryParam('pid1') != '') s.eVar25 = s.getQueryParam('pid1'); s.prop25 = "D=v25";		// pid1: example assigning values
    s.eVar25 = s.getValOnce(s.eVar25, 's_var_25', 0)
    if (s.getQueryParam('pid2') != '') s.eVar26 = s.getQueryParam('pid2'); s.prop26 = "D=v26";		// pid2  example assigning values
    s.eVar26 = s.getValOnce(s.eVar25, 's_var_26', 0)
    ...
    */
    ...
    //s.pageName+= ':' + detallePagina;				// Manual formation s.PageName: "seccion:subseccion:subsubseccion:detallePagina"
    s.pageName = s.getPageName();																		// Plugin getPageName()
    ...
    s.prop14 = "colombia";											    							// Pais del Medio
    s.prop15 = "america";  																			// Zona (Region) : ONLY RADIO
    s.prop17 = "web";																					// Canal
    s.prop18 = "prisa";																				// Organizacion
    s.prop19 = "wradio";																				// Producto
    s.prop20 = "wradio.com.co";																		// Dominio
    s.prop21 = s.getNewRepeat();   																	// Usuario Nuevo
    s.prop22 = "convencional";																		// Formato : ONLY RADIO
    s.prop24 = hours + ":" + minutes + ":" + seconds;															// Set hh:mm:ss (12:32:48)
    s.prop29 = 'D=c15+":"+c14+":"+c19+":"+c22+":"+c17+":"+c20';										// Combination segmentation : ONLY RADIO
    s.prop28 = reg;																					// Registrado / No Registrado
    s.prop30 = "radio";																				// Unidad de negocio
    s.prop31 = "informacion"; 																		// Tematica
    s.prop33 = s.getVisitNum();																		// Visit Number By Month : ONLY RADIO
    s.prop34 = userId;   																				// ID Usuario Registrado
    ...
    //s.prop39 = detallePagina; 																		// Titulo página
    s.prop44 = s.getTimeParting('h', gmt);				 											// Set hour (12:00PM)
    s.prop45 = "español"; 																			// Idioma : ONLY RADIO
    ...
    if (s.prop1) s.eVar1 = "D=c1"		// subseccion (prop1) = seccion + subseccion
    if (s.prop2) s.eVar6 = "D=c2"		// subsubseccion (prop2) = seccion>subseccion>subsubseccion
    if (s.prop4) s.eVar10 = "D=c4"; 	// tema (prop4) = seccion>subseccion>subsubseccion>tema
    if (s.prop5) s.eVar5 = "D=g"		// URL
    ...
    if (s.prop11) s.eVar11 = "D=c11"; 	// subtema  (prop11) = seccion>subseccion>subsubseccion>tema>subtema
    if (s.prop12) s.eVar12 = "D=c12"; 	// subtema1 (prop12) = seccion>subseccion>subsubseccion>tema>subtema>subtema1
    if (s.prop14) s.eVar14 = "D=c14"	// Pais del Medio
    if (s.prop15) s.eVar15 = "D=c15"	// Zona (Region) : RADIO
    ...
    if (s.prop21) s.eVar21 = "D=c21"	// Usuario Nuevo
    if (s.prop22) s.eVar24 = "D=c22"	// Formato
    if (s.prop24) s.eVar59 = "D=c24"	// Set hour:minutes:seconds (12:32:48)
    if (s.prop28) s.eVar22 = "D=c28"	// Registrado / No Registrado
    if (s.prop29) s.eVar31 = "D=c29"	// Combination segmentation : mymajorcompany
    ...
    if (s.prop34) s.eVar43 = "D=c34"	// ID Usuario Registrado
    ...
    if (typeof (document.title) != 'undefined' && document.title != '') {
        ...
        s.prop39 = s.prop39.toLowerCase().split('|')[0];
        s.eVar39 = "D=c39";
    } else {
        s.prop39 = "";
        s.eVar39 = "D=c39";
    ...
    if (s.prop45) s.eVar45 = "D=c45"	// Idioma
    if (s.prop60) s.eVar60 = "D=c60"	// Días desde última visita


    // Set Page View Event, Add event2 only if it does not exist
    if(typeof(s.events)!=="undefinded"){
...
// function search
function searchOMN(query_str, query_count) {
    s = s_gi(s_accountF)
    ...
    s.tl(this, 'o', "searchOMN");
}

// function audio
function audioOMN(accion, player, reproduccion, nombreAudio, tiempoAudio) {
    s.usePlugins = false;
    s = s_gi(s_account)
    s.linkTrackVars = "events,eVar2,eVar4,eVar8,eVar9,eVar17,eVar20,eVar30,eVar35,eVar42,eVar45,eVar47,eVar48,eVar68,eVar70";
    s.linkTrackEvents = 'event16,event17,event13,event14';
    switch (accion) {
        case "inicio":
            s.eVar8 = nombreAudio;
            s.events = "event16";
    ...
    s.eVar17 = "web";											// Canal
    s.eVar20 = "wradio.com.co";   							// Dominio
    s.eVar30 = "radio";   									// Unidad de Negocio
    s.eVar35 = s.getTimeParting('h', gmt);					// Set hour (12:00PM)
    s.eVar42 = reproduccion;									// Tipo reproduccion (vod,streaming)
    s.eVar48 = s.getTimeParting('d', gmt); 					// Set day (Jueves)
    s.eVar70 = "audio";
    s.eVar74 = tiempoAudio;									// Tiempo Audio
    s.tl(this, 'o', "audioOMN");
    s.manageVars("clearVars");
}

// function video
function videoOMN(accion, player, reproduccion, nombreVideo, tiempoVideo) {
    s.usePlugins = false;
    s = s_gi(s_account)
    s.linkTrackVars = "events,eVar2,eVar4,eVar8,eVar9,eVar17,eVar20,eVar30,eVar35,eVar42,eVar45,eVar47,eVar48,eVar68,eVar70";
    ...
    s.eVar48 = s.getTimeParting('d', gmt); 					// Set day (Jueves)
    s.eVar70 = "video";
    s.eVar74 = tiempoVideo;									// Tiempo Audio
    s.tl(this, 'o', "videoOMN");
    s.manageVars("clearVars");
}
...
function facebookTrack() {
    s = s_gi(s_accountF);
    s.linkTrackVars = "eVar3,eVar39,eVar69,events";
    s.linkTrackEvents = "event69";
    s.events = 'event69';
    s.eVar3 = s.pageName;
    s.eVar39 = document.title.toLowerCase();
    s.eVar69 = "facebook";
    s.tl(this, 'o', 'fb:like');
}
