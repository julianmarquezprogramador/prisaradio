...
//var s= new AppMeasurement(getAnalyticsAccount());
//s_account= "097d06709f13c1565915a5c7017019ad9846390b";
...
function omn_asyncPV(){
    ...
    if(section=="widget"){
        ...
        if(/cadenadial.com/.test(parentDomain)){
            s.abort = true;
        }
    }
...
//if (typeof(adblock_enabled) == 'undefined')adblock_enabled='D="sin_ADBLOCK-"+User-Agent';
...
// Reading Article (page):
    ...
    var regexpNoticia = /http.\/\/([^\/]*)\/(\d+)\/.*\.html/i;

    result_re = regexpNoticia.exec(document.location.href);
    if (result_re){
...
if(omn_isPlayer()){//detect, is player
    section="player";
}
s.channel = section;
s.pageName= s.siteID + location.pathname;														// Get Path Name (On)
...
if(typeof tucu !== 'undefined'){
    if(typeof tucu.dev !== 'undefined'){
        if(tucu.dev == true){
            ...
            console.log("feature: Include AMP into product");
            console.log("////////////////////////////////////////////////////");
        }
        else{
            console.log(dtm_version);
