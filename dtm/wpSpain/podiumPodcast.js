...
if (typeof(episode_name) == 'undefined') episode_name='';
...
// s.pageName+= '/' + pageName;			       													// Manual formation s.pageName (off)
    ...
    if(section==""){ //this rule is when take a file from mediateca, then the section = widget;
        if(((window.frames.pageName).indexOf("instanceplayer.prisasd.com/widget"))>-1){
            section="widget";
        }
    }
// External widget
    if(section=="widget"){
        ...
        if(/.podiumpodcast.com/.test(parentDomain)){
            s.abort = true;
        }
    }
    ...
    s.prop39= omn_catchFirsElementOfTitle(document.title, "|");
    ...
    s.prop71=season;																										// Season
    s.prop72=episode;																										// Episode
    s.prop73=episode_name;																							// Episode Name
    ...
    if(typeof arrayAmp !== 'undefined') {
        if(arrayAmp['amp']=="true"){
            s.prop17="amp";
            omn_cleanTitle_withoutSeo();
            //s.prop20= arrayAmp["server"];
            s.prop20= omn_deleteWWW(arrayAmp["server"]);
            s.prop39= omn_catchFirsElement(arrayAmp['title']);
            s.prop45= cadena_titulo_limpio;
            //s.pageURL= omn_cleanUrl(arrayAmp["pageURL"]);
            s.pageURL= omn_cleanUrl(arrayAmp["source_url"]);
            s.pageName= omn_deleteWWW(arrayAmp["server"]) + omn_cleanUrl(arrayAmp["source_path"]);
    ...
    if(s.prop13)s.eVar13="D=c13"  											// Program
    ...
    if(s.prop71)s.eVar71="D=c71"												// Season
    if(s.prop72)s.eVar72="D=c72"												// Episode
    if(s.prop73)s.eVar73="D=c73"												// Episode Name
...
//function for cleaning title of the V39
function omn_cleanTitle_withoutSeo(){
...
if(typeof tucu !== 'undefined'){
    if(typeof tucu.dev !== 'undefined'){
        if(tucu.dev == true){
            ...
            console.log("bugs: when it was widget into vice, a pixel was sent to prisacompodiumpodcast");
            console.log("solved: now send to multidistribución");
            console.log("bugs: v39 and v45 were not shown");
            console.log("solved: added code for v39 and v45");
            console.log("Added code for AMP into of Product")
            console.log("////////////////////////////////////////////////////");
        }
        else{
            console.log(dtm_version);
